...
from model import Heuristics, authorial_1,authorial_1_lr, benders,kinable_homo,kinable_homo_lr, lagrange
...
if __name__ == '__main__':
    ...
    while instances:
        ...
        # if j != 20: continue
        # if (i,j,k) != (4,20,5): continue
        # if j != 10: continue
        # if (i,j,k) != (3,10,4): continue
        # if j != 5: continue
        # if (i,j,k) != (1,5,2): continue
        ...
        # md = authorial_1_lag.md(dt)
        # hybrid = Heuristics.HybridBendersHeuristic(dt)
        # hybrid.run()
        # hybrid = Heuristics.HybridBendersHeuristic(dt)
        # hybrid.run()
        ...
        # md.run(dt)
        # bender = benders.AdvancedBenders(dt)
        # bender.run()
        # ui = goat_benders.BendersDecompositionSolver(dt)
        # ui.run()
        # input()
        ...
        # md.run(dt)
        # input()
        # print("dt.md_authorial_1_lag_cost",dt.md_authorial_1_lag_cost,'time: ',round(dt.md_authorial_1_lag_time,2),'gap: ',dt.md_authorial_1_lag_gap)
        # print("dt.md_authorial_cost",dt.md_authorial_1_cost,'time: ',round(dt.md_authorial_1_time,2),'gap: ',dt.md_authorial_1_gap)
        # print(i, j, k)
        # input()

        # if (i,j,k) != (2,10,2): continue
        # # if j != 5: continue
        # md = authorial_1.md(dt)
        # md.run(dt)
        # md = kinable_homo.md(dt)
        ...
        # print("dt.md_authorial_cost",dt.md_authorial_1_cost,'time: ',round(dt.md_authorial_1_time,2),'gap: ',dt.md_authorial_1_gap)
        # input()
        if dt.md_authorial_1_lr_cost is None: 
            md = authorial_1_lr.md(dt)
            md.run(dt)
        ...
        if dt.md_authorial_1_cost is None: 
            ...
            md.run(dt)
            dt.save_in_json('kinable')
        if dt.md_kinable_homo_lr_cost is None: 
