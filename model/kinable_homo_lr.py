...
class md():
    ...
    def create_model(self):
        ...
        m.add_constraints(m.sum(x[(i, j, k)] for (i,j) in outgoing(A,code) for k in K ) == y[vValueToTrip[code][0]]  for code in D_codes)
    ...
    def extract_solution(self, dt: data.Instance, sol):
            ...
            dt.md_kinable_homo_lr_solution = unorganized_solution
            dt.md_kinable_homo_lr_nb_nodes = sol.solve_details.nb_nodes_processed
            dt.md_kinable_homo_lr_best_integer = sol.solve_details.best_bound
            dt.md_kinable_homo_lr_time = sol._solve_details.time
            dt.md_kinable_homo_lr_cost = sol.objective_value

        
    def run(self, dt: data.Instance):
        self.create_model()
        print('criado o modelo kinable homo  LR')
        ...
        match jobStatus:
            case 'UNKNOWN':
                ...
                dt.md_kinable_homo_lr_gap = None
                dt.md_kinable_homo_lr_cost = -1
            case 'OPTIMAL_SOLUTION':
                if dt.LOG: print('solução otima')
                dt.md_kinable_homo_lr_gap = 0
                self.extract_solution(dt, sol)
            case 'FEASIBLE_SOLUTION':
                if dt.LOG: print('solução subótima ')
                dt.md_kinable_homo_lr_gap = round(jobDetails.gap*100, 2)
                self.extract_solution(dt, sol)
            case 'INFEASIBLE_SOLUTION':
                if dt.LOG: print('sem solução viável ')
                dt.md_kinable_homo_lr_gap = None
        dt.md_kinable_homo_lr_status = jobDetails.status
        if dt.LOG: 
            print('PAROU POR ', dt.md_kinable_homo_lr_status)
            print('fo final: ', dt.md_kinable_homo_lr_cost)
            print('tempo de execução: ', dt.md_kinable_homo_lr_time )
