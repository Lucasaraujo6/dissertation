...
class md():
    ...
    def create_model(self):
        ...
        # # 3 e 4 restrição de time lag máximo  - a chegada anterior mais o tempo de descarregamento deve ser ter no max 5 de intervalo com a posterior
        ...
        # 5 - conservação de fluxo: para uma viagem, a planta de origem e destino deve ser a mesma
        ...
        # m.add_constraints(m.sum(x[i,j,l,r] for (i,j,l,r) in IJLR if r in range(rr - t[i][j], rr+1)  )+
        #             m.sum(y[i,j,l,r] for (i,j,l,r) in IJLR if r in range( rr - t[i][j] - unloadingTime, rr+1) ) <= 2*vehiclQnt for rr in T) 
        # rr+1,rr-t-Q
        # 6 - Para cada instante, toma-se a quantidade de viagens terminadas antes do instante e 
        ...
        # 7 NÃO PODE TER MAIS DO QUE OS VEHICS DISPONÍVEIS NAS PLANTAS
        m.add_constraints(m.sum(x[i,j,l,r] for (i,j,l,r) in IJLR if rr < r < rr +t[i][j] )+
                            m.sum(y[i,j,l,r] for (i,j,l,r) in IJLR if rr - t[i][j] - unloadingTime <= r <= rr ) <= vehiclQnt for rr in T) 
    ...
    def extract_solution(self, dt: data.Instance, sol):
        ...
        dt.md_authorial_1_solution = vehicles_trips
        print('dt.md_authorial_1_solution',dt.md_authorial_1_solution)
        dt.md_authorial_1_nb_nodes = sol.solve_details.nb_nodes_processed
        print('dt.md_authorial_1_nb_nodes',dt.md_authorial_1_nb_nodes)
        dt.md_authorial_1_best_integer = sol.solve_details.best_bound
        print('dt.md_authorial_1_best_integer',dt.md_authorial_1_best_integer)
        dt.md_authorial_1_time = sol._solve_details.time
        print('dt.md_authorial_1_time',dt.md_authorial_1_time)
        dt.md_authorial_1_cost = sol.objective_value
        print('dt.md_authorial_1_cost',dt.md_authorial_1_cost)
    ...
    def run(self, dt: data.Instance):
        ...
        match jobStatus:
            ...
            case 'INFEASIBLE_SOLUTION':
                ...
                dt.md_authorial_1_gap = 100
        dt.md_authorial_1_status = jobDetails.status
        if dt.LOG: print('PAROU POR ', dt.md_authorial_1_status)
        
        if dt.LOG: 
            print('fo final: ', dt.md_authorial_1_cost)
            print('tempo de execução: ', dt.md_authorial_1_time )
