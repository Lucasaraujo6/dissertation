...
class md():
    ...
    def extract_solution(self, dt: data.Instance, sol):
            ...
            dt.md_kinable_homo_solution = unorganized_solution
            dt.md_kinable_homo_nb_nodes = sol.solve_details.nb_nodes_processed
            dt.md_kinable_homo_best_integer = sol.solve_details.best_bound
            dt.md_kinable_homo_time = sol._solve_details.time
            dt.md_kinable_homo_cost = sol.objective_value
    ...
    def run(self, dt: data.Instance):
        ...
        match jobStatus:
            case 'UNKNOWN':
                ...
                dt.md_kinable_homo_gap = None
                dt.md_kinable_homo_cost = -1
            case 'OPTIMAL_SOLUTION':
                if dt.LOG: print('solução otima')
                dt.md_kinable_homo_gap = 0
                self.extract_solution(dt, sol)
            case 'FEASIBLE_SOLUTION':
                if dt.LOG: print('solução subótima ')
                dt.md_kinable_homo_gap = round(jobDetails.gap*100, 2)
                self.extract_solution(dt, sol)
            case 'INFEASIBLE_SOLUTION':
                if dt.LOG: print('sem solução viável ')
                dt.md_kinable_homo_gap = None
        dt.md_kinable_homo_status = jobDetails.status
        if dt.LOG: 
            print('PAROU POR ', dt.md_kinable_homo_status)
            print('fo final: ', dt.md_kinable_homo_cost)
            print('tempo de execução: ', dt.md_kinable_homo_time )
