...
class md():
    ...
    def create_model(self):
        ...
        m.maximize(m.sum(h[j]* dt.d[j]  for (j) in J)
                # + m.sum( 
                # m.sum(y[i,j,l,r] for (i,j,l,r) in IJLR if i == ii and r + t[i][j] + unloadingTime <= rr) 
        ...
        #restrição de time lag mínimo  e máximo
        ...
        # m.add_constraints( m.sum(y[i,j,l,r] for (i,j,l,r) in IJLR if i == ii and  r+t[i][j] + unloadingTime <=rr and r in range(dt.a[j],dt.b[j]-unloadingTime+1)) + z[ii]
        #                 - m.sum(x[i,j,l,r] for (i,j,l,r) in IJLR if i == ii and r-t[i][j]<= rr and r in range(dt.a[j],dt.b[j]-unloadingTime+1)) 
        #                 >= 0
        #                 for ii in I for rr in T )
    ...
    def extract_solution(self, dt: data.Instance, sol):
        ...
        dt.md_authorial_1_lag_solution = vehicles_trips
        print('dt.md_authorial_1_lag_solution',dt.md_authorial_1_lag_solution)
        dt.md_authorial_1_lag_nb_nodes = sol.solve_details.nb_nodes_processed
        print('dt.md_authorial_1_lag_nb_nodes',dt.md_authorial_1_lag_nb_nodes)
        dt.md_authorial_1_lag_best_integer = sol.solve_details.best_bound
        print('dt.md_authorial_1_lag_best_integer',dt.md_authorial_1_lag_best_integer)
        dt.md_authorial_1_lag_time = sol._solve_details.time
        print('dt.md_authorial_1_lag_time',dt.md_authorial_1_lag_time)
        dt.md_authorial_1_lag_cost = sol.objective_value
        print('dt.md_authorial_1_lag_cost',dt.md_authorial_1_lag_cost)
    ...
    def run(self, dt: data.Instance):
        ...
        match jobStatus:
            ...
            case 'INFEASIBLE_SOLUTION':
                ...
                dt.md_authorial_1_lag_gap = 100
        dt.md_authorial_1_lag_status = jobDetails.status
        if dt.LOG: print('PAROU POR ', dt.md_authorial_1_lag_status)
        
        if dt.LOG: 
            print('fo final: ', dt.md_authorial_1_lag_cost)
            print('tempo de execução: ', dt.md_authorial_1_lag_time )
